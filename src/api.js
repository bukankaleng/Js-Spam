...
class LineAPI {
  ...
  setTHttpClient(options = {
    ...
    //options.headers['X-Line-Application'] = 'DESKTOPWIN\t8.3.0RFU-BOT\t18.99';
  ...
  _qrCodeLogin() {
    ...
    this._client.getAuthQrcode(true, '⚜ Created by : Basyir',(err, result) => {
      const qrcodeUrl = `line://au/q/${result.verifier}`;
      qrcode.generate(qrcodeUrl,{small: true});
      console.info(`⚜ Script spam js\n⚜ CREATED BY : TRIO PEKOK BOTS\n\n⚜link qr: ${qrcodeUrl}`)
      ...
      this._authService.loginZ(reqx,(err,success) => {
        ...
        const authToken = config.tokenn;
          const certificate = config.certificate;
                ...
                this.options.path = this.config.LINE_COMMAND_PATH;
                this.setTHttpClient(this.options);
          this.options.headers['User-Agent'] = 'Line/8.3.3';
          this.axz = true;
          this.setTHttpClient(this.options);
          this.axz = false;
         ...
         try{
           ...
           (err,success) => {
             ...
             this._client.pinCode = success.pinCode;
                     console.info("\n\n⚜ Masukkan Pincode "+success.pinCode+"\nUntuk verifikasi email");
